...
function buildSchemaConstraintSummary(): string {
  return Object.entries(TOOL_REGISTRY)
    .map(([sheet, entry]) => {
      ...
      return [
        `${sheet}`,
        `- allowed operations: ${entry.schema.tooling.allowedOperations.join(', ')}`,
        `- allowed selectors: ${entry.schema.tooling.allowedSelectors.join(', ')}`,
        insertRequired.length > 0
          ? `- required on insert: ${insertRequired.join(', ')}`
          : '- required on insert: none beyond generated defaults',
